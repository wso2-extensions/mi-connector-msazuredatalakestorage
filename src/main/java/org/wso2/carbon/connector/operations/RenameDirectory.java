...
public class RenameDirectory extends AbstractConnector {
    ...
    public void connect(MessageContext messageContext) {
        ...
        Object newDirectoryName = messageContext.getProperty(AzureConstants.NEW_DIRECTORY_NAME);

        if (fileSystemName == null || directoryName == null || newDirectoryName == null) {
            AzureUtil.setErrorPropertiesToMessage(messageContext, Error.MISSING_PARAMETERS,
                    "Mandatory parameter [fileSystemName] or [directoryName] or [newDirectoryName] cannot be empty.");
            handleException("Mandatory parameter [fileSystemName] or [directoryName] or [newDirectoryName] cannot be empty.",
                    messageContext);
        }
        ...
        try {
            ...
            AzureStorageConnectionHandler azureStorageConnectionHandler = (AzureStorageConnectionHandler) handler
                    .getConnection(AzureConstants.CONNECTOR_NAME, connectionName);
            ...
            if (dataLakeDirectoryClient.exists()) {
                dataLakeDirectoryClient.rename(fileSystemName.toString() , newDirectoryName.toString());
                status = true;
            }
    ...
    private void generateResults(MessageContext messageContext, boolean status) {
        String response = AzureUtil.generateResultPayload(status,
                !status ? AzureConstants.ERR_DIRECTORY_DOES_NOT_EXIST : "");
