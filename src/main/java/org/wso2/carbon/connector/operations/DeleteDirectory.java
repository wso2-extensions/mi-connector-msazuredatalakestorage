...
public class DeleteDirectory extends AbstractConnector {
    ...
    public void connect(MessageContext messageContext) {
        ...
        try {
            ...
            if(dataLakeDirectoryClient.exists()) {
                dataLakeDirectoryClient.deleteRecursively();
                status = true;
            }
    ...
    private void generateResults(MessageContext messageContext , boolean status) {
        String response = AzureUtil.generateResultPayload(
                status , !status ? AzureConstants.ERR_DIRECTORY_DOES_NOT_EXIST : "");
